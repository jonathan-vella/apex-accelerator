...
> ⚠️ **Re-entry guard**: If apex-azure-prepare was invoked as a **resume** from a specialized skill (e.g., apex-azure-cloud-migrate), **skip this check** and go directly to Step 4.

| User prompt mentions                                                        | Action                                                                                                                                 |
| --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Lambda, AWS Lambda, migrate AWS, migrate GCP, Lambda to Functions           | **Invoke apex-azure-cloud-migrate skill NOW** → then resume apex-azure-prepare at Step 4                                                         |
| Azure Functions, function app, serverless function, timer trigger, func new | Stay in **apex-azure-prepare**. When selecting compute, **prefer Azure Functions** templates and best practices, then continue from Step 4. |
...
└── Migrate/modernize for Azure
    ├── Cross-cloud migration (AWS/GCP/Lambda)? → **Invoke apex-azure-cloud-migrate skill** (do NOT continue in apex-azure-prepare)
