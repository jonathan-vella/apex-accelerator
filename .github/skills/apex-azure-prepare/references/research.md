...
`apex-microsoft-docs` for remaining documented service questions. If a required
...
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **Hosting**                  |                                                                                                                         |                                                                                                                                  |
| Container Apps               | [Container Apps](services/container-apps/README.md)                                                                     | `apex-azure-diagnostics`, `azure-observability`, `azure-nodejs-production`                                                            |
| App Service                  | [App Service](services/app-service/README.md)                                                                           | `apex-azure-diagnostics`, `azure-observability`, `azure-nodejs-production`                                                            |
...
| PostgreSQL                   | —                                                                                                                       | —                                                                                                                                |
| Storage (Blob/Files)         | [Storage](services/storage/README.md)                                                                                   | `apex-azure-storage`, `azure-security-hardening`                                                                                      |
| **Messaging**                |                                                                                                                         |                                                                                                                                  |
...
| Managed Identity             | —                                                                                                                       | `azure-security`, `apex-entra-app-registration`                                                                                       |
| **Observability**            |                                                                                                                         |                                                                                                                                  |
| Application Insights         | [App Insights](services/app-insights/README.md)                                                                         | `appinsights-instrumentation` (invoke for instrumentation)                                                                       |
| Log Analytics                | [App Insights](services/app-insights/README.md) | `apex-azure-diagnostics` for log troubleshooting; `apex-microsoft-docs` for setup |
| Azure Data Explorer (ADX)    | — | `apex-azure-kusto` for ADX database queries and analytics |
| **AI Services**              |                                                                                                                         |                                                                                                                                  |
...
| Scenario                                        | Action                                                                                                                                                                              |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Migrating AWS Lambda / cross-cloud workload** | **Invoke `apex-azure-cloud-migrate`** (assessment + code conversion, then resume apex-azure-prepare)                                                                                          |
| Using Azure Functions                           | Stay in **apex-azure-prepare** — load [selection.md](services/functions/templates/selection.md) → Follow [composition.md](services/functions/templates/recipes/composition.md) algorithm |
...
| Greenfield or proposed-resource pricing         | `cost-estimate-subagent` via the [pricing parent contract](../../apex-azure-defaults/references/cost-estimate-parent-contract.md); no deployed scope required |
| Existing deployment spend or rightsizing        | `apex-azure-cost-optimization` with actual cost/utilization evidence |

**Skill/Reference Invocation Pattern:**

KQL syntax alone is not a routing signal: Log Analytics/App Insights incidents
belong to `apex-azure-diagnostics`; ADX databases belong to `apex-azure-kusto`. Do not query
...
Add research findings to `infra/{iac}/{project}/.azure/plan.md` under a `## Research Summary` section with source references and key insights per component.
