<!-- ref:as-built-from-azure-v1 -->
# As-Built Documentation from Existing Azure Deployment

Routing source: [Local adapter](../../../../tools/apex-prompts/utility-prompts/as-built-from-azure.prompt.md).
...
Generate comprehensive as-built documentation (all Step 7 documents plus a Python diagram)
...
to produce the full Step 7 documentation suite plus a Python diagram.
...
- Phase 6 handed off to selected `08-As-Built` and the `07-*.md` documents plus the
  Python architecture diagram source with PNG/SVG renders exist.
...
- After loading required procedure guidance and verifying routing, `askQuestions`
  is the first task action. No discovery reads, searches, or shell commands until
  the Phase 1-2 questions are complete; Phase 3 then verifies access.
...
- State initialized or recovered via `apex-recall`, without fabricated completion
- `agent-output/{project}/01-requirements.md` through `06-deployment-summary.md`
  as pseudo-artifacts derived from discovery
- `agent-output/{project}/07-*.md` and `07-ab-diagram.{py,png,svg}`
...
- All as-built documents plus a Python diagram will be generated

---

## Phase 1: Environment Discovery — CALL `askQuestions` FIRST

After loading required guidance, call `askQuestions` before discovery reads,
searches, or commands. Complete Phase 1-2 questions before Phase 3 access checks.
...
Run `apex-recall show {project} --json`. Recover existing work before considering
`apex-recall init {project} --json`. Record discovered facts and their provenance
through supported recall commands. Never write session-state JSON directly.
Synthesized artifacts are evidence, not completed steps, review passes, or human
approvals. Reconcile missing reviews and approvals through the named owners
before any standard workflow transition. Do not bypass challenger gates to
force Step 7 readiness.
...
Treat Phase 4.6 as preliminary inventory only. Stop and request the named
`04g-Governance` owner for authoritative effective-policy discovery, including
inherited assignments, definitions and exemptions. Produce `.md` and `.json`
constraints there; if unavailable, leave governance blocked rather than
labeling a subscription-only assignment list as complete. List:
...
Create `agent-output/{project}/README.md` distinguishing discovered/synthesized
evidence from reviewed and approved workflow completion. List missing gates.
...
request manual selection of `08-As-Built` with its exact configured model and
tools. Do not invoke it as an unnamed worker or inherit the intake model:
...
> the Python as-built diagram with PNG and SVG renders.
...
| File                            | Source      | Content                                    |
| ------------------------------- | ----------- | ------------------------------------------ |
| Recall-managed state           | This procedure | Discovered evidence and unresolved gates |
...
| `07-ab-diagram.{py,png,svg}`    | 08-As-Built | Reproducible architecture diagram          |
