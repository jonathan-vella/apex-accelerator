<!-- ref:review-imported-iac-v1 -->
# Review Imported IaC

Routing source: [Local adapter](../../../../tools/apex-prompts/utility-prompts/review-imported-iac.prompt.md).
...
2. Run `apex-recall show {project} --json`; recover existing state or initialize
  a genuinely new project with `apex-recall init {project} --json`. Do not edit
  session-state JSON directly or mark unreviewed intake artifacts complete.
...
1. Stop and request manual selection of `04g-Governance` with its configured
  model and tools, passing the confirmed scope and imported-IaC paths.
2. If that owner is unavailable, record governance as blocked. Do not substitute
  inline discovery or an unnamed subagent.
...
1. Stop and request manual selection of `03-Architect` with its configured model
...
2. If the user still declines to provide them, record WAF review as blocked with
  the missing evidence. Static findings can be returned, but are not a substitute
  for the named Architect's assessment or required independent cost review.
...
- Use only named, permitted validation workers; use human owner selection for WAF assessment.
