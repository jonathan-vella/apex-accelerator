...
- Plan an Azure Container Registry in the IaC that apex-azure-prepare generates
- Import images from GCR/ECR/Docker Hub (`az acr import`) or rebuild and push (`docker build` → `docker push`)
  once the registry exists and the owner approves
- Pull with managed identity (`AcrPull`), not registry admin credentials

### Phase 4: Plan the Container Apps Deployment
