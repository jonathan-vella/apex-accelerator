...
When gathering AKS diagnostic evidence, prefer `mcp_azure-mcp_aks`, then the smallest discovered AKS-MCP tool that fits the read, then supporting Azure tools such as `mcp_azure-mcp_applens`, `mcp_azure-mcp_monitor`, or `mcp_azure-mcp_resourcehealth`. Use raw `az aks` and `kubectl` only when the AKS-MCP surface cannot perform the needed check.
