...
`--dry-run`; `--approve` (required to run; the script exits 3 without it). Default timeout by gadget name: `snapshot_*`/`top_*` → 5s,
...
- IG gadgets only read state, but `run-ig` creates a privileged debug pod on the node.
- Invoke gadgets through `run-ig` (`scripts/run-ig.sh` / `scripts/run-ig.ps1`); it resolves the node and applies the correct timeout. Run it with `--dry-run`, show the user the command, and add `--approve` only after they approve.
- The debug pod (`node-debugger-*`) stays after the gadget exits. Delete it after collecting output, with the user's approval.
