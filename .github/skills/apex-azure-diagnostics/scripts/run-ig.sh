...
# Without --approve the script stops before running it; --dry-run needs no approval.
# kubectl leaves the node-debugger-* pod behind: list it with
# `kubectl get pods -A | grep node-debugger` and delete it with approval afterwards.
#
# Usage:
...
#   --approve              Confirm the user approved the privileged debug pod
#
# Examples:
#   ./run-ig.sh --gadget trace_dns --pod web-0 --ns default --approve
#   ./run-ig.sh --gadget snapshot_process --node aks-nodepool1-1234 --approve
#   ./run-ig.sh --gadget tcpdump --pod web-0 --ns default --pf "port 80" --approve
#   ./run-ig.sh --gadget traceloop --pod web-0 --ns default --filter --syscall-filters --filter open,connect --approve
...
if [[ "$APPROVED" != "true" ]]; then
    echo "Refusing to create a privileged debug pod without explicit approval." >&2
    echo "Show the command above to the user, then re-run with --approve." >&2
    exit 3
fi
