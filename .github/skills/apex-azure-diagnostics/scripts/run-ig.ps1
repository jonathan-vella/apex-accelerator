...
.DESCRIPTION
  ...
  Without -Approve the script stops before running it; -DryRun needs no approval.
  kubectl leaves the node-debugger-* pod behind: find it with
  `kubectl get pods -A | Select-String node-debugger` and delete it with approval afterwards.
...
.PARAMETER Approve
  Confirm the user approved the privileged debug pod.

.EXAMPLE
  ./run-ig.ps1 -Gadget trace_dns -Pod web-0 -Namespace default -Approve

.EXAMPLE
  ./run-ig.ps1 -Gadget snapshot_process -Node aks-nodepool1-1234 -Approve

.EXAMPLE
  ./run-ig.ps1 -Gadget tcpdump -Pod web-0 -Namespace default -Pf "port 80" -Approve

.EXAMPLE
  ./run-ig.ps1 -Gadget traceloop -Pod web-0 -Namespace default -Filter --syscall-filters,open,connect -Approve
...
if (-not $Approve) {
    Write-Error 'Refusing to create a privileged debug pod without explicit approval. Show the command above to the user, then re-run with -Approve.'
    exit 3
}
