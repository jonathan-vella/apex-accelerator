...
This file covers **global / platform-level** probe checks (Azure Front Door, Traffic Manager, Application Insights connectivity). For service-specific health-probe checks and target settings, see:
...
> Azure Container Apps per-service references are planned upstream but not yet shipped.
