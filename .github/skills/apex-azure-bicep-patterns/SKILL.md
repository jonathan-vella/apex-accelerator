...
`iac-bicep-best-practices.instructions.md` (style) and `apex-azure-defaults` skill (naming, tags, regions).

> **Canonical sources** — the security baseline, AVM-first mandate, naming
> conventions, required tags, and unique-suffix rule live in
> [`apex-azure-defaults/SKILL.md`](../apex-azure-defaults/SKILL.md) and
...
4. **Pin AVM versions** — query MCR at plan time and follow the `apex-azure-defaults` live-resolution and exception policy
5. **Add diagnostics + budget** — every deployed resource gets diagnostics; every deployment implements the governed cost-monitoring contract
...
- **Budget**: Follow the canonical cost-monitoring contract; amount and notification inputs MUST be parameters
