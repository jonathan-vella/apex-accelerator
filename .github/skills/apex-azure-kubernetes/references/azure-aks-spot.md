<!-- ref:azure-aks-spot-v1 -->

# AKS Spot Node Pools

Recommend Spot VM node pools for batch, dev/test, or fault-tolerant workloads. Spot capacity is discounted and
can be evicted at any time; price it through `cost-estimate-subagent`.

> The `az aks` and `kubectl apply` commands below change the cluster. This skill doesn't run them: they describe
> the target configuration for the IaC owner (06b/06t) or an approved change.
